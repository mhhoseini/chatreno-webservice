 ...
 */
class Financialapproval extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        if($this->B_user->checkrequestip('financialapproval',$command,get_client_ip(),50,50)) {
            ...
            {//register marketer
                $employeetoken=checkpermissionemployeetoken($employee_token_str,'modify','financialapproval');
                ...
                if($employeetoken[0]=='ok')
                {
                    ...
                    }else if($approvalmode=='okchecked')	{
                    ...
                    foreach($result as $row)
                    {
                        ...
                        if($row['request_agent_id']!=null || $row['request_agent_id']!=""){
                            ...
                            //****************************************************************************
                            $image=$this->B_db->get_image($agent['agent_image_code']);
                            //*******************************************************************
                ...
                if($command=="okfinancialapproval")
                {
                    $employeetoken=checkpermissionemployeetoken($employee_token_str,'modify','financialapproval');
                    $request_id=$this->post('request_id') ;
                    $request_financial_approval_desc=$this->post('request_financial_approval_desc') ;
                    $request_financial_approval_price=$this->post('request_financial_approval_price') ;
                    $request_financial_approval_difference_price=$this->post('request_financial_approval_difference_price') ;
                    if($employeetoken[0]=='ok')
                    {

                        $query="DELETE FROM request_financial_approval_tb WHERE request_financial_approval_request_id=$request_id";
                        $this->B_db->run_query_put($query);
                        $query="INSERT INTO request_financial_approval_tb( request_financial_approval_request_id, request_financial_approval_employee_id, request_financial_approval_date,request_financial_approval_difference_price, request_financial_approval_desc, request_financial_approval, request_financial_approval_price) VALUES
                                  (                   $request_id         , ".$employeetoken[1]."                  , now()                        ,'$request_financial_approval_difference_price','$request_financial_approval_desc',       1                ,'$request_financial_approval_price')";
                        $this->B_db->run_query_put($query);
                        echo json_encode(array('result'=>"ok"
                        ,"data"=>""
                        ,'desc'=>' تاییدیه مالی درخواست ثبت شد'),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
                    ...
                    if($command=="notfinancialapproval")
                    {
                        $employeetoken=checkpermissionemployeetoken($employee_token_str,'modify','financialapproval');
                        $request_id=$this->post('request_id') ;
                        $request_financial_approval_desc=$this->post('request_financial_approval_desc') ;
                        ...
                        if($employeetoken[0]=='ok')
                        {
                            ...
                            $query="INSERT INTO request_financial_approval_tb( request_financial_approval_request_id, request_financial_approval_employee_id, request_financial_approval_date,request_financial_approval_difference_price, request_financial_approval_desc, request_financial_approval, request_financial_approval_price) VALUES
                                                             (                   $request_id         , ".$employeetoken[1]."                  , now()                          ,'$request_financial_approval_difference_price','$request_financial_approval_desc',       0                ,'$request_financial_approval_price')";
                            $this->B_db->run_query_put($query);
                            echo json_encode(array('result'=>"ok"
                            ,"data"=>$query
                            ,'desc'=>'تاییدیه مالی درخواست رد شد'),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
