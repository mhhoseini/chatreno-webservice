 ...
 */
class Price_buildingquality extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        if ($this->B_user->checkrequestip('price_buildingquality', $command, get_client_ip(),50,50)) {
            if ($command == "get_price") {
                ...
                $fieldinsurance = 'buildingqualityins';

                $query5 = "select * from buildingquality_costcons_tb where buildingquality_costcons_id=$buildingquality_costcons_id";
                $result5 = $this->B_db->run_query($query5);
                $buildingquality_costcons = $result5[0];
 ...
 AND fieldcompany_deactive=0
  ORDER BY company_id ASC";
                ...
                foreach ($result as $row) {
                    ...
                    $query4 = "select * from buildingquality_price_tb 
where
  buildingquality_price_deactive=0
 AND buildingquality_price_fieldcompany_id=" . $row['fieldcompany_id'] . "";
                    $result4 = $this->B_db->run_query($query4);
                    $num4 = count($result4[0]);
                    if ($num4 > 0) {
                        $buildingquality_price = $result4[0];
                        $price = ($buildingquality_price['buildingquality_price_buildpercent'] * $buildingquality_costcons['buildingquality_costcons_price'] * $buildingquality_area) / 1000000;
                        $price = ($price * 109) / 100;
