 ...
 */
class Financialpaying2 extends REST_Controller {
    ...
    public function index_post()
    {
        ...
        if ($this->B_user->checkrequestip('financialapproval', $command, get_client_ip(),50,50)) {
            ...
            {//register marketer
                ...
                {
                    $query1="select * from request_tb,company_tb,request_state,fieldinsurance_tb,user_tb,requst_ready_tb,user_address_tb 
                    where user_id=request_user_id AND fieldinsurance=request_fieldinsurance AND request_state_id=request_last_state_id AND company_id=request_company_id 
                    AND request_id=requst_ready_request_id AND user_address_user_id=user_id  ";
                    $query1.=" AND (request_last_state_id=10 OR request_last_state_id=11)";

                    $query2="select count(*) AS cnt from request_tb,company_tb,request_state,fieldinsurance_tb,user_tb,requst_ready_tb,user_address_tb 
                    where user_id=request_user_id AND fieldinsurance=request_fieldinsurance AND request_state_id=request_last_state_id AND company_id=request_company_id 
                    AND request_id=requst_ready_request_id AND user_address_user_id=user_id  ";
                    ...
                    $request_financial_doc_id_query = " ";
                    if(!empty($_REQUEST['request_financial_doc_id'])){
                        $request_financial_doc_id=$this->post('request_financial_doc_id');
                        $request_financial_doc_id_query.=" AND request_financial_doc_id=".$request_financial_doc_id."";
                    }

                    $request_ready_clearing_mode_id_query = " ";
                    if(!empty($_REQUEST['request_ready_clearing_mode_id'])){
                        $request_ready_clearing_mode_id=$this->post('request_ready_clearing_mode_id');
                        $request_ready_clearing_mode_id_query.=" AND request_ready_clearing_mode_id=".$request_ready_clearing_mode_id."";
                    }
...
//******************************************************************************************
                        ...
                        //***************************************************************************************************************
                        //***************************************************************************************************************
                        $query6=" SELECT * FROM requst_ready_tb,request_ready_clearing_mode_tb WHERE request_ready_clearing_mode_id=request_ready_clearing_id AND requst_ready_request_id=".$request_id.$request_ready_clearing_mode_id_query;
                        ...
                        request_financial_paying_request_id=".$request_id.$request_financial_doc_id_query;
		 ...
		 where request_financial_paying_request_id=request_id AND request_agent_id=".$agent_id.") ";
                        ...
                        foreach($result as $row)
                        {
                            ...
                            AND  request_financial_doc_id=".$row['request_financial_doc_id']."".$request_financial_doc_id_query;
                            $result2=$this->B_db->run_query($query2);
                            $agent=$result2[0];
                            $record['agent_id']=$agent['agent_id'];
                            $record['agent_name']=$agent['agent_name'];
