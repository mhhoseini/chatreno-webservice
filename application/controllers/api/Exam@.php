    ...
    public function migrate_post()
    {
        ...
        foreach($sanads as $sanad){
            ...
            foreach($records as $rec){
                $str_procedure[$sanad['sanad_qroup']][]=$rec['SalMali'].",".$rec['Shobe'].",".$last_code.",'".$rec['Sharh']."','".$rec['sanad_head']."','".$rec['Tarikh']."',".$rec['CodeKol'].",
                ...
                //$kol[$rec['CodeKol']] = $rec['CodeKol'];
                //$moin[$rec['CodeMoin']."-".$rec['CodeKol']."-".$rec['CodeTaf']]= $rec['CodeMoin']."-".$rec['CodeKol']."-".$rec['CodeTaf']."-".$rec['CodeTaf2'];
                //$list_sql[$sanad['sanad_qroup']][] = "select * FROM SarFasl where CodeKol In (".implode(',',$kol).")";
            }
            //echo implode(',',$moin);
            //echo $list_sql[$sanad['sanad_qroup']][] = "select * FROM SarFasl where CodeKol In (".implode(',',$kol).")";
            $last_code = $last_code + 1;
        }
        //print_r($moin);
        //echo implode(',',$moin);
    ...
    public function sarfasl_taf_post(){
        ...
        if(!empty($result)){
            foreach ($result as $obj)
            {
                    ...
                    echo $query = 'INSERT INTO [Accounting].[SarFasl_TafFs] (Shobe, CodeTaf_Group, CodeTaf, CodeTaf2_Group, CodeTaf2, Sharh, Mahiat, Arz, Disable, IsMeghdari)
                ...
                if ($conn){
                    try{
                           if(($result = sqlsrv_query($conn,$query)) !== false){
                                ...
                                $query = "Update sarfasl_taffs set RowID=".$last_id.",IsMigrated=1,Migrate_Date=now() where SarfaslTaffs_ID IN(".$obj['SarfaslTaffs_ID'].")";
